...
export async function handler(chatUpdate) {
    ...
    try {
        ...
        if (!m.fromMem && m.text.match(/(gata|des|able|izo|hir|.-.|._.|:)|:(|:v|v:|o.o|;v|v;|v':|:'v)/gi)) {
...
case 'remove':
    if (chat.welcome) {
        ...
        for (let user of participants) {
            ...
            break
        case 'promote':
            text = (chat.sPromote || this.spromote || conn.spromote || '@user is now administrator')
        case 'demote':
...
≡ 𝘿𝙀𝙏𝙀𝘾𝙏𝙀𝘿 𝘼 𝘿𝙀𝙇𝙀𝙏𝙀𝘿 𝙈𝙎𝙂
*┌─⊷* 𝘼𝙉𝙏𝙄 𝘿𝙀𝙇𝙀𝙏𝙀 
✮͢ *𝙉𝙐𝙈𝘽𝙀𝙍 :* @${participant.split`@`[0]} 
✮͢  𝙏𝙃𝙀 𝙈𝙀𝙎𝙎𝘼𝙂𝙀 𝙄𝙎 𝘿𝙊𝙒𝙉
└────────────────────────
`.trim(), msg, {
...
global.dfail = (type, m, conn) => {
    let msg = {
        rowner: '_👑This command can only be used by *Dipak*_',
        owner: '_👑This command can only be used by *Dipak*_',
        mods: '🔰  This function is only for *moderators*',
        premium: '💠 ```This is only for premium user  if you want premium type /owner```',
        ...
        unreg: '📇 Please register to continue :\n\n*/reg name.age*\n\n📌Example : /reg Dipak★.69',
