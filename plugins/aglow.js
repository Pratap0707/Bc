let handler = async (m, { conn, args, text, usedPrefix, command }) => {
   ...
   let tee = `😾Enter a short text\n\n📌 Example  : *${usedPrefix + command}* MAAHIR`
   let too = `😫 Separate the text with a *+* \n\n📌 Example : \n*${usedPrefix + command}* GLITCH *+* MD`
    m.react(rwait)
...
switch (type) {
	...
	conn.sendFile(m.chat, di, 'logo.png', `MADE WITH ❤️ BY GLITCH MD`, m)
	m.react(done)
	break 
     case 'ninja': 
	if (!text) throw tee
	let mh = global.API('fgmods', '/api/textpro/ninja', { text }, 'apikey')
	conn.sendFile(m.chat, mh, 'logo.png', `MADE WITH ❤️ BY GLITCH MD`, m)
	m.react(done)
	break 
     case 'tiktok': 
	if (!text) throw tee
	let ch = global.API('fgmods', '/api/textpro/tiktok', { text }, 'apikey')
	conn.sendFile(m.chat, ch, 'logo.png', `MADE WITH ❤️ BY GLITCH MD`, m)
	m.react(done)
	break  
     case 'lightglow': 
	if (!text) throw tee
	let ut = global.API('fgmods', '/api/textpro/lightglow', { text }, 'apikey')
	conn.sendFile(m.chat, ut, 'logo.png', `MADE WITH ❤️ BY GLITCH MD`, m)
	m.react(done)
	break 
     case 'layer': 
	if (!text) throw too
	...
	conn.sendFile(m.chat, lr, 'logo.png', `MADE WITH ❤️ BY GLITCH MD`, m)
	m.react(done)
	break  
     case 'aglow': 
	if (!text) throw tee
	let da = global.API('fgmods', '/api/textpro/advancedglow', { text }, 'apikey')
	conn.sendFile(m.chat, da, 'logo.png', `MADE WITH ❤️ BY GLITCH MD`, m)
	m.react(done)
	break 
     case 'diamond': 
	if (!text) throw tee
	let de = global.API('fgmods', '/api/textpro/diamond', { text }, 'apikey')
	conn.sendFile(m.chat, de, 'logo.png', `MADE WITH ❤️ BY GLITCH MD`, m)
	m.react(done)
	break 
  default:
} 
