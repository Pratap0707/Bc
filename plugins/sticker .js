...
let handler = async (m, { conn, args, usedPrefix, command }) => {
  ...
  try { 	
    ...
    if (/webp|image|video/g.test(mime)) {
      ...
      if (!img) throw `😺 Reply to an image or video with*${usedPrefix + command}*`
      let out
      try {
    ...
    else throw 'Conversion failed, try sending first *image/video/gif* and then respond with the command'
  }
}
